...
import com.stericson.RootTools.containers.AccessMode;
import com.stericson.RootTools.containers.FileInfo;
...
//    public static String utilPath;
    ...
    public static File combinePaths(File parent, String child)
    {
    ...
    {
        if (paths == null || paths.length == 0) { throw new IllegalArgumentException("paths"); }

        File fullPath = new File(paths[0]);

        for (int i = 1; i < paths.length; ++i)
        {
            fullPath = RootTools.combinePaths(fullPath, paths[i]);
        }
     ...
     */
    @Nullable
    public static String getUtilPath(String util)
    {
        return getInternals().getUtilPath(util);
    }
    ...
    {
        return getInternals().findBinaryPaths(binaryName);
    }
    ...
    {
        return getInternals().getFileInfo(file);
    }
...
//        }
        ...
        return RootTools.getFileInfo(file.getPath());
    }
    ...
    {
        return getInternals().newSymLink(target, name);
    }
    ...
    public static void setPermissions(File file, AccessMode user, AccessMode group, AccessMode others) throws IOException, RootDeniedException, TimeoutException
    {
        if (!file.exists())
        {
            throw new FileNotFoundException(file.getPath());
        ...
        RootTools.getInternals().setFilePermissions(file.getPath(), new Permissions(user, group, others));
    }

    public static void setPermissions(String path, AccessMode user, AccessMode group, AccessMode others) throws IOException, RootDeniedException, TimeoutException
    {
    ...
    {
        RootTools.setPermissions(file, AccessMode.valueOf(user), AccessMode.valueOf(group), AccessMode.valueOf(others));
    }

    public static void setPermissions(String path, int user, int group, int others) throws IOException, RootDeniedException, TimeoutException
    {
        RootTools.setPermissions(new File(path), user, group, others);
    }

    public static void setUserPermissions(String path, int userAccess) throws IOException, RootDeniedException, TimeoutException
    {
        Permissions permissions = RootTools.getFileInfo(path).getPermissions();
        RootTools.setPermissions(new File(path), userAccess, permissions.getGroupAccess().getValue(), permissions.getOthersAccess().getValue());
    }

    public static void setGroupPermissions(String path, int groupAccess) throws IOException, RootDeniedException, TimeoutException
    {
        Permissions permissions = RootTools.getFileInfo(path).getPermissions();
        RootTools.setPermissions(new File(path), permissions.getUserAccess().getValue(), groupAccess, permissions.getOthersAccess().getValue());
    }

    public static void setOthersPermissions(String path, int othersAccess) throws IOException, RootDeniedException, TimeoutException
    {
        Permissions permissions = RootTools.getFileInfo(path).getPermissions();
        RootTools.setPermissions(new File(path), permissions.getUserAccess().getValue(), permissions.getGroupAccess().getValue(), othersAccess);
    }
}
