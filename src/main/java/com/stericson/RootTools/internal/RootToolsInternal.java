...
import com.stericson.RootTools.lib.RootToolsLib;
...
public final class RootToolsInternal
{
    private static final String[] _BinPaths = {
        "/sbin/",
        "/system/bin/",
        "/system/xbin/",
        "/data/local/xbin/",
    ...
    {
        return RootToolsInternal._BinPaths;
    }

    @NonNull
    public static List<SymbolicLink> getSymLinks(@NonNull String directory, int maxDepth) throws TimeoutException, RootDeniedException, IOException
    {
     ...
     */
    @NonNull
    public static List<String> findBinaryPaths(final String binaryName)
    {
//        boolean found = false;
...
//        try
//        {
//            for (final String path : _BinPaths)
//            {
...
//        }
        ...
        if (pathList.isEmpty())
        {
            ...
            for (String binPath : _BinPaths)
            {
                FileStat fileStat = RootToolsInternal.getFileStat(new File(binPath, binaryName).getPath());
...
//        return found;
    ...
    {
        List<String> binaryPaths = RootToolsInternal.findBinaryPaths(binaryName);
...
//        binaryPaths.addAll(RootTools.lastFoundBinaryPaths);
    ...
    /**
     * Use this to check whether or not a path OR directory exists on the filesystem.
     *
     * @param path String that represent the path OR the directory, including the full path to the
     *             path and its name.
     * @return a boolean that will indicate whether or not the path exists.
     */
    public static boolean exists(@NonNull final String path, @Nullable FileType fileType)
    {
        if (fileType == null)
        {
            try
        ...
        FileStat fileStat = RootToolsInternal.getFileStat(path);
        return fileStat != null && fileStat.type == fileType;
    }

    @NonNull
    public static String resolvePath(String path) throws TimeoutException, RootDeniedException, IOException
    {
        for (String line : SimpleCommand.run("realpath " + path))
        {
            return line;
        }
    ...
    @NonNull
    public static File resolvePath(File path) throws IOException, RootDeniedException, TimeoutException
    {
        return new File(RootToolsInternal.resolvePath(path.getPath()));
    }

    public static void assertExists(String path, FileType fileType) throws FileNotFoundException
    {
        if (!RootToolsInternal.exists(path, fileType))
        {
            throw new FileNotFoundException(path);
        }
    }

    public static void assertExists(File file, FileType fileType) throws FileNotFoundException
    {
        assertExists(file.getPath(), fileType);
    }
    ...
    /**
     * @param path String represent the file system path.
    ...
    //    try
    //    {
    //        return RootToolsLib.getFileStat(path, followLinks);
    //    }
...
//                            filePermission = permissions.getValue();
     ...
     */
    public static long getConvertedSpace(String spaceStr)
    {
        try
        {
    ...
    public static void setFilePermissions(String path, Permission user, Permission group, Permission others) throws TimeoutException, RootDeniedException, IOException
    {
        String commandLine = "chmod " + user.toOctalString() + group.toOctalString() + others.toOctalString() + " " + path;
        Log.d("RootToolsInternalMethods.setFilePermissions()", commandLine);
    ...
    public static ArrayList<Mount> getMounts() throws Exception
    {
        ...
        shell.add(cmd);
        RootToolsInternal.commandWait(shell, cmd);
     ...
     */
    public static boolean isBinaryAvailable(Context context, String binaryName)
    {
        Installer installer;
